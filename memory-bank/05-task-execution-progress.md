...
## Token-budget checkpoint (2026-02-20, TASK-015 near-complete)
- Причина фиксации:
  - Контекст текущей сессии превысил лимит 350k токенов (сработал `Token Budget Gate` из `.clinerules/10-memory-bank-workflow.md`).
- Что уже выполнено по `TASK-015`:
  - Выполнена корректировка интеграционного теста
    `practice_lesson_should_support_all_question_types_and_partial_scoring`
    в `CourseLessonCrudIntegrationTest`:
    - ожидание `maxPoints` приведено к фактическому контракту статистики (`1` вместо `5`).
  - Валидация пройдена:
    - `mvn -pl monolith-mvp -Dtest=CourseLessonCrudIntegrationTest test`
    - результат: `BUILD SUCCESS`, `Tests run: 6, Failures: 0, Errors: 0`.
  - Backlog обновлён:
    - `memory-bank/tasks.json`: `TASK-015` переведена в `done`.
  - `memory-bank/02-active-context.md` обновлён под текущее состояние `TASK-015`.
- Что осталось сделать после `/newtask`:
  1. Добавить итоговую запись о завершении `TASK-015` в `memory-bank/06-system-development-progress.md`.
  2. Отправить пользователю финальный отчёт по выполненной задаче.
