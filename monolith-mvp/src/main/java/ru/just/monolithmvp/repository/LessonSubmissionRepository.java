...
public interface LessonSubmissionRepository extends JpaRepository<LessonSubmission, Long> {
    ...
    @Query("""
        select s
        from LessonSubmission s
        where s.student.id = :studentId and s.lesson.position < :position and s.completed = true
        """)
    List<LessonSubmission> findAllCompletedByStudentIdAndLessonPositionLessThan(Long studentId, Integer position);
}
